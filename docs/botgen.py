...
def main(args):
    ...
    if args.file:
        with open(args.file, 'w') as fd:
            ...
            if args.continues:
                while True:
                    time.sleep(args.write_period)
                    do_generate(fd)
                    fd.flush()
                    print("Write to file: ", args.file)
            fd.write("]")
    elif args.directory:
        if args.continues:
            while True:
                time.sleep(args.write_period)
                file = open(args.directory + "data" + str(counter) + ".json", "w+")
                counter += 1
                do_generate(file)
        else:
            file = open(args.directory + "data.json", "w+")
            do_generate(file)
    else:
        do_generate()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dir', '--directory', type=str, default=None, help="directory for file spamming")
    parser.add_argument('-c', '--continues', type=bool, default=False, help="continues generation")
    parser.add_argument('-wt', '--write-period', type=int, default=2, help="write period in milliseconds")
